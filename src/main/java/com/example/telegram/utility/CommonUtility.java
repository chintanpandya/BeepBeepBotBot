...
public class CommonUtility {
    ...
    public static String formateTime(String time) throws ParseException{
       SimpleDateFormat sdf=null;
       try{
         sdf = new SimpleDateFormat((time.length() == 2 ? "mm" : (time.length()==3 ? "Hmm" : "HHmm")));
         Date dt = sdf.parse(time);
         sdf = new SimpleDateFormat("HH:mm aa");
         return sdf.format(dt);
       }catch(ParseException e){
           sdf = new SimpleDateFormat((time.length() == 1 ? "H" : (time.length() == 2 ? "mm" : (time.length()==3 ? "Hmm" : "HHmm"))));
